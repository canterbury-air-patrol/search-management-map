...
class OrganizationTestCase(TestCase):
    ...
    def test_organization_create(self):
        ...
        json_data = self.create_organization(organization_name=alt_organization_name, client=self.smm.client1)
    ...
    def test_organization_lists(self):
        ...
        org_list = self.get_all_organizations(client=self.smm.client1)
        self.assertEqual(len(org_list), 0)
        org_list = self.get_all_organizations(client=self.smm.client2)
        self.assertEqual(len(org_list), 0)
        # Create an organization for each user
        org1 = self.create_organization(organization_name='org1', client=self.smm.client1)
        org2 = self.create_organization(organization_name='org2', client=self.smm.client2)
        ...
        org_list = self.get_my_organizations(client=self.smm.client1)
        self.assertEqual(len(org_list), 1)
        self.assertEqual(org_list[0]['name'], 'org1')
        self.assertEqual(org_list[0]['role'], 'Admin')
        org_list = self.get_my_organizations(client=self.smm.client2)
        self.assertEqual(len(org_list), 1)
    ...
    def test_organization_user_add(self):
        ...
        org = self.create_organization(organization_name='org', client=self.smm.client1)
        self.add_user_to_org(organization=org, user=self.smm.user2.username, client=self.smm.client1)
        org_list = self.get_my_organizations(client=self.smm.client2)
        self.assertEqual(len(org_list), 1)
        ...
        org = self.create_organization(organization_name='org2', client=self.smm.client2)
        # Check user1 can't see it currently
        org_list = self.get_my_organizations(client=self.smm.client1)
        self.assertEqual(len(org_list), 1)
        self.assertEqual(org_list[0]['role'], 'Admin')
        # Check user1 can't add themselves
        organization_user_add_url = f"/organization/{org['id']}/user/{self.smm.user1.username}/"
        response = self.smm.client1.post(organization_user_add_url, {'role': 'M'})
        # should get a 404 if they aren't in the organization
        self.assertEqual(response.status_code, 404)
        org_list = self.get_my_organizations(client=self.smm.client1)
        self.assertEqual(len(org_list), 1)
        # Add user1
        self.add_user_to_org(organization=org, user=self.smm.user1.username, client=self.smm.client2)
        org_list = self.get_my_organizations(client=self.smm.client1)
        self.assertEqual(len(org_list), 2)
    ...
    def test_organization_user_remove(self):
        ...
        self.del_user_from_org(organization=org, user=self.smm.user2.username, client=self.smm.client1)
        org_list = self.get_my_organizations(client=self.smm.client2)
        self.assertEqual(len(org_list), 0)
        ...
        response = self.smm.client2.get(organization_details_url)
        self.assertEqual(response.status_code, 200)
        response = self.smm.client2.get(organization_details_url, headers={'HTTP_ACCEPT': 'application/json'})
        self.assertEqual(response.status_code, 200)
    ...
    def test_organization_asset_add_list(self):
        ...
        """
        org = self.create_organization(organization_name='org', client=self.smm.client1)
        asset_list = self.get_org_asset_list(organization=org, client=self.smm.client1)
        ...
        org2 = self.create_organization(organization_name='org2', client=self.smm.client1)
        asset_list = self.get_org_asset_list(organization=org2, client=self.smm.client1)
        self.assertEqual(len(asset_list), 0)
        # Check non-members can't add assets
        asset2 = self.create_asset(name='asset2', asset_type=asset_type, owner=self.smm.user2)
        self.add_asset_to_organization(asset=asset2, organization=org2, client=self.smm.client2, expected_status=404)
        # Check only the asset owners can add assets
        self.add_asset_to_organization(asset=asset2, organization=org2, client=self.smm.client1, expected_status=403)
        # Check general members cant add assets
        self.add_user_to_org(user=self.smm.user2, organization=org2, role='M')
        self.add_asset_to_organization(asset=asset2, organization=org2, client=self.smm.client2, expected_status=403)
        # Check that admins who didn't create the org can add assets
        self.add_user_to_org(user=self.smm.user2, organization=org2, role='A')
        self.add_asset_to_organization(asset=asset2, organization=org2, client=self.smm.client2)
    ...
    def test_organization_details(self):
        ...
        org = self.create_organization(organization_name=org_name, client=self.smm.client1)
